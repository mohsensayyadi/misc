...
def scoreRF(dfObj, TopKFeatures):
    featuresList = list(dfObj.columns)
    featuresList.remove('EXP_HHI')
    featuresList.remove('user_id')
    featuresList.remove('zipcode')
    ...
    weights = np.where(train_y == 0, 1, train_y)
    weights = np.where(train_y == 1, 2, weights)
    weights = np.where(train_y == 2, 4, weights)
    #weights = np.where(train_y == 3, 9, weights)
    #weights = np.where(weights == 4, 27, weights)
    print(train_x.shape, train_y.shape, test_x.shape, test_y.shape)

    model = RandomForestClassifier(random_state=0)
    model.fit(one_hot_data, dfObj['label'])
    model.fit(train_x, train_y, weights)

    pred = model.predict(test_x)
    print("Accuracy", accuracy_score(test_y, pred))
    print("Macro F1", f1_score(test_y, pred, average='macro'))
    print("Precision on each class", precision_score(test_y, pred, average=None))
    print("Recall on each class", recall_score(test_y, pred, average=None))
...
query = """
SELECT t1.* except(app_profile, demographics, Conversion, attribution, app_ids, clusters, makes, report_date, latest_date, latest_ts, latest_days_ago), 
t2.* except(user_id, EXP_HHI, EXP_HHI_int), t3.* except (user_id)
FROM `sc-targeting-measurement.mosen_dev.HHI_features_raw_sparse` t1
left outer join `sc-targeting-measurement.mosen_dev.HHI_category_counter_features_20210707` t2 using(user_id)
left outer join `sc-targeting-measurement.mosen_dev.HHI_features_SCC_aggr` t3 using(user_id)
"""
...
#train_x, train_y, test_x, test_y, model =scoreRF(df1, 5)
...
suffix = 'HHI_RF_scc_allData_fixed_3classes_July11'
d = {"descr": "Xgboost with 4 classes on all data"}
d['model'] = model
with open('HHI_model_'+suffix+'.pickle', 'wb') as handle:
    pickle.dump(d, handle, protocol=pickle.HIGHEST_PROTOCOL)

plot_confusion_matrix(model, test_x, test_y, normalize='true')
plt.savefig('confMatrix_'+suffix+'.png')
#plt.show()